...
def is_draw_epoch(cur_epoch):
    """Determines if the model should be evaluated at the current epoch."""
    return (
            (cur_epoch + 1) % cfg.train.draw_period == 0 or
            (cur_epoch + 1) == cfg.optim.max_epochs
    )
...
def get_ckpt_dir():
    return f'{cfg.run_dir}/checkpoints'
...
def load_pretrained(
    ...
    pretrained = torch.load(pretrained_model_path, map_location=torch.device(cfg.device))
...
def save_ckpt(
    ...
    ckpt_name = get_ckpt_epoch(epoch)
    torch.save(ckpt, get_ckpt_path(ckpt_name))
